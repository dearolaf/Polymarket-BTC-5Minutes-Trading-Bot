...
def get_current_mode(client):
    ...
    try:
        mode = (
            client.get('btc_trading:simulation_mode')
            or client.get('sol_trading:simulation_mode')
        )
        if mode is None:
            return None
        return mode == '1'
...
def display_status(client):
    ...
    print("\n" + "=" * 60)
    print("BTC 5-MIN BOT - CURRENT STATUS")
    print("=" * 60)
...
def main():
    ...
    print("\n" + "=" * 60)
    print("BTC 5-MIN BOT - SIMULATION MODE CONTROL")
    print("=" * 60)
