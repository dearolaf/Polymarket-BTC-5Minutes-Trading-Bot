...
BTC_UPDOWN_INTERVAL_SEC = 300  # 5-minute markets
...
def current_btc_5m_slug() -> str:
    """
    Current BTC 5-minute up/down market slug: btc-updown-5m-{unix_start}.
    """
    now = datetime.now(timezone.utc)
    unix_s = int(now.timestamp())
    iv = BTC_UPDOWN_INTERVAL_SEC
    interval_start = math.floor(unix_s / iv) * iv
    slug = f"{BTC_UPDOWN_SLUG_PREFIX}-{interval_start}"
...
def get_next_btc_5m_markets(count: int = 3) -> list[str]:
    """Next N BTC 5-minute market slugs (current interval first)."""
    unix_s = int(datetime.now(timezone.utc).timestamp())
    iv = BTC_UPDOWN_INTERVAL_SEC
    interval_start = math.floor(unix_s / iv) * iv
    slugs = [f"{BTC_UPDOWN_SLUG_PREFIX}-{interval_start + i * iv}" for i in range(count)]
...
current_sol_5m_slug = current_btc_5m_slug
get_next_sol_5m_markets = get_next_btc_5m_markets
