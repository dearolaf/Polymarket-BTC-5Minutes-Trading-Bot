...
class PerformanceTracker:
    ...
    def __init__(
        ...
        # Order lifecycle counters for execution client callbacks (placed / filled / rejected)
        self._order_events: Dict[str, int] = {}
       
        logger.info(f"Initialized Performance Tracker (capital=${initial_capital})")


    def record_order_event(self, event_type: str) -> None:
        """Increment counters when the execution client reports order events."""
        key = (event_type or "").lower().strip()
        if not key:
            return
        self._order_events[key] = self._order_events.get(key, 0) + 1
        logger.debug(f"Order event recorded: {key} | counts={self._order_events}")
