...
def apply_gamma_markets_patch():
    ...
    try:
        ...
        async def _load_all_using_gamma_markets(self, filters: dict | None = None) -> None:
            ...
            try:
                # Many slug= params in one GET → huge URL; HTTP stack may fail ("error sending request").
                chunk_size = max(5, min(25, int(os.getenv("GAMMA_SLUG_CHUNK", "15"))))
                slug_val = filters.get("slug")
                if slug_val is not None and isinstance(slug_val, (tuple, list)) and len(slug_val) > chunk_size:
                    slugs = list(slug_val)
                    self._log.info(
                        f"Batching Gamma slug query: {len(slugs)} slugs in chunks of {chunk_size}"
                    )
                    markets: list = []
                    seen: set[str] = set()
                    for start in range(0, len(slugs), chunk_size):
                        batch_filters = dict(filters)
                        batch_filters["slug"] = tuple(slugs[start : start + chunk_size])
                        batch_filters["limit"] = max(
                            50, min(500, len(batch_filters["slug"]) * 3)
                        )
                        chunk = await gamma_markets.list_markets(
                            http_client=self._http_client,
                            filters=batch_filters,
                            timeout=120.0,
                        )
                        for m in chunk:
                            key = m.get("slug") or m.get("conditionId") or ""
                ...
                for market in markets:
                    try:
                        ...
                        if slug.lower().startswith(f"{_pfx}-"):
